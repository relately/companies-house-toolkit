...
describe(
  ...
  () => {
    it('should produce output the same as the Companies House API', async () => {
      await syncProducts(['prod100', 'prod101', 'prod183', 'prod217/2024']);

      console.log('Done syncing');

      const options = [
        'snapshot',
        `--product-183-path=${process.cwd()}/src/test/uat/data/prod183`,
        `--product-101-path=${process.cwd()}/src/test/uat/data/prod101`,
        `--product-100-path=${process.cwd()}/src/test/uat/data/prod100`,
        `--product-217-path=${process.cwd()}/src/test/uat/data/prod217`,
      ...
      const { stdout } = await runCli(options);
